...
const ctx = await b.newContext({ viewport: { width: 1600, height: 900 }, deviceScaleFactor: 8 })
...
await (await p.$('[data-plan-unfold]')).screenshot({ path: `${OUT}/glyph-unfold.png` })
await p.click('[data-plan-unfold]'); await p.mouse.move(8, 8); await p.waitForTimeout(900)
await (await p.$('[data-plan-unfold]')).screenshot({ path: `${OUT}/glyph-fold.png` })
await b.close()
