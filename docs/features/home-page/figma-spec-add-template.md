...
### 14.3-bis ⚠️ 26.08.2026 (ночь) — СЖАТИЕ СТАЛО РАМПОЙ, а не снапом

Числа §14.2/§14.3 не изменились — изменилось, ЧЕМ они достигаются. Дизайнер сообщил баг
(«когда скролишь, список дергается резко»); замер показал, что снап 215 → 146 в одном
коммите двигает сетку на 69 px, которых юзер не просил (худший кадр 16.6 px при
`ΔscrollTop = 0`). Теперь высота шапки — функция скролла: `подошва = 215 − 69·min(S/69, 1)`,
шапка — оверлей над скроллером, её бывшая высота — `padding-top` скроллера. Оба
нарисованных состояния достигаются точно (`scrollTop 0` → 215/131/первый ряд 215;
`scrollTop 69` → 146/78/146), а промежуточные кадры — линейная интерполяция тех же
расстояний. Закон, приёмка по четырём способам ввода и цена — `design-system.md` §5.

Что это меняет для чтения бордов: прокрученный борд соответствует **`scrollTop ≥ 69`**, а
не 21. Комментарий §14.2 о «21 px» описывал, при каком офсете дизайнер снял кадр — при
рампе полное сжатие приходится на 69 px скролла, потому что подошва обязана ехать вместе с
первым рядом. Это единственное сознательное расхождение с прочтением борда, и оно
вынужденное: любая более короткая рампа отрывает подошву от контента.
...
* **⚑ ГДЕ СТОИТ ПИЛЮЛЯ `Preview` — ОТКРЫТЫЙ ВЫБОР (два собранных варианта, кадры готовы).**
  Первый срез ставил её в угол без собственной земли, и на светлом рисунке она садилась на
  копирайт сайта, обрезая его посреди слова. Собрано два варианта: **A** — угол + короткая
  нижняя вуаль (та же плашка, что дизайнер завёл под «+», повёрнутая на 90°, с землёй через
  `--card-ground` и альфой 84 %); **B** — пилюля по центру рисунка, без вуали. Кадры на шести
  названных рисунках (AURA / ODEON / Lumen светлые, KORE / forge / IRONSIDE тёмные) плюс
  самый «шумный» кроп библиотеки: `scratchpad/wp3/A-*` и `wp3/B-*`. Замеры и рекомендация
  (A) — `design-system.md` §5 «Два варианта посадки пилюли». Ключевой довод против B: она не
  убирает коллизию, а переносит её в фокус картинки — на ODEON разрезает «SUMMER», на
  PayNexus накрывает ценник.
* **⚑ Глиф пилюли — четыре угловые скобки, не диагональные стрелки.** На 14 px две стрелки
  сливаются в двунаправленную «resize», а рядом со словом `Preview` стрелка читается как
  «перейти». Скобки — знак «на весь экран» из любого видеоплеера. Наше решение, кадры для
  сверки: `wp3/glyph-a-diagonal-arrows.png` против `wp3/glyph-b-corner-brackets.png`.
