...
* **(в) нужен ли «+» в доке?** Дизайнер сказал «в списке», а в доке у той же карточки
  другое действие (начать сайт, а не приложить к промпту). В доке «+» НЕ поставлен;
  плашка там — только рецепт, унаследованный от `--card-ground` (проверено: резолвится
  в `#09090b`, ровно фон страницы).
