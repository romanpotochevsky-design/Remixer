...
### `icann-verify` — подтверждение почты регистранта

**Итерация 1 по достижимости. ⚠️ Число не проверено — не отгружать цифру.**

**Когда возникает.** ICANN 2013 RAA требует подтвердить email регистранта после новой
регистрации или смены контакта. Не подтвердили — **домен приостанавливают**: падает и
сайт, и почта. Это самое разрушительное неучтённое состояние во всём флоу.

**Нарисовано** на борде `28206:66756` как `5 icann verify`: «We sent a link to
roman@example.com · 14 days left» + `Resend`.

⚠️ **Правило реальное, число — нет.** «15 дней» в нашем ресёрче прослеживается до правила
отвязки **Squarespace**, а не до страницы DreamHost или ICANN. **Не отгружать цифру**,
пока кто-нибудь не прочитает статью DreamHost о registrant verification. До тех пор копирайт
должен говорить «before the deadline in the email», а не изобретать обратный отсчёт.
