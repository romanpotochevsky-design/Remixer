...
>
> 🔴 **Поправка 14.09.2026 (первопартийная):** домен с неподтверждённой почтой
> регистранта **не резолвится**, поэтому «`live` с неподтверждённой почтой» — НЕ состояние
> этой машины. Затронуты: §2 (карта), §3 (достижимость), §`securing`, §`live`, §5 целиком и
> §6 (чеклист). Факт — `docs/knowledge/product-facts.md` п. 29, решение — `decisions.md` 14.09.2026.
>
...
> 🔴 **ПОПРАВКА 14.09.2026 — в карте не хватает одного блокирующего состояния.**
> Домен, купленный у нас, **не резолвится, пока клиент не подтвердил почту регистранта**
> (девелопер DreamHost, Slack, 14.09.2026 — `product-facts.md` п. 29). Значит на пути
> покупки между `registering` и всем, что за ним, стоит **`waiting-on-confirmation`**
> (бывший `icann-verify`, §5), и он гатит переход в `securing`/`live`:
>
> ```
> registering --> waiting_on_confirmation : первая регистрация на этот email
> waiting_on_confirmation --> propagating  : кликнул в письме
> waiting_on_confirmation --> suspended    : 16-й день (состояние не написано)
> ```
>
> **У `live` теперь ДВА предусловия:** адрес резолвится **И** почта регистранта
> подтверждена. «`live` с неподтверждённой почтой» — **не состояние этой машины**.
> Саму диаграмму выше НЕ правили: она описывает **подключение** и старше пути покупки
> (§5 дописан туда же позже, и три его состояния в карту тоже не внесены) — перерисовка
> всей карты под оба пути — отдельная задача, и её стоит делать вместе с дизайнером.
...
| **`waiting-on-confirmation`** (бывший `icann-verify`) — *добавлено 14.09.2026* | — | — | — | — | — | **1** — только покупка и только первая регистрация на этот email; **блокирует всё ниже себя** |
...
🔴 **Дописано 14.09.2026.** Правило «сертификат нельзя выпустить, пока домен не
указывает на DreamHost» имеет новое следствие: домен с неподтверждённой почтой
регистранта **не резолвится** (`product-facts.md` п. 29), а значит и проверка владения
пройти не может — **в это состояние такой домен, по-видимому, вообще не попадает**.
Это **вывод, а не факт** (девелопера про сертификат не спрашивали) — открытый вопрос №2 в
§`icann-verify`. Пока он не закрыт, приоритет слота решён в пользу почты: она блокирует
замок, а не ждёт за ним.
...
🔴 **Дописано 14.09.2026 — у этого состояния есть второе предусловие.** На пути покупки
`live` наступает только при **подтверждённой почте регистранта**: домен с
неподтверждённой почтой не резолвится (`product-facts.md` п. 29), то есть строка чеклиста
`Connected to your site` ложна, а заголовок «Your site is live at {domain}» — прямая неправда.
Клиент в этот момент находится в `waiting-on-confirmation`, а не в `live` с оговоркой.
...
> 🔴 **И с 14.09.2026 разница не только в скорости.** Купленный домен не работает
> вообще, пока клиент не подтвердил почту регистранта (девелопер DreamHost, Slack —
> `product-facts.md` п. 29). Значит у Flow B есть шаг, которого у Flow A нет и который
> **проходит не машина, а человек в своём почтовом ящике** — §`waiting-on-confirmation` ниже.
...
🔴 **И у чеклиста есть стадия «минус один» (14.09.2026).** На пути покупки до
подтверждения почты регистранта не закрывается **ни одна** из трёх строк: домен не
резолвится (`product-facts.md` п. 29), значит `Connected to your site` ложна, а
`Security (SSL) on` недостижима вслед за ней. Чеклист в этом состоянии лучше НЕ показывать
вовсе: три пустые строки читаются как «идёт работа», а работа не идёт — ждут клиента.
Это ответ дизайна, а не факт.
...
| **1а** — *вставлено 14.09.2026* | **`waiting-on-confirmation`** | **1** | Не редкость: это видит **каждый**, кто покупает у нас домен впервые, и без этого экрана купленный домен просто не работает без объяснения. Борд `28206:66756` есть, но говорит не то |
...
## 🔴 Поправка 14.09.2026 (вечер) — стадий подключения ТРИ, сертификата среди них нет

Дизайнер назвал модель прямо: *«у нас есть только 3 стадии/статуса при подключении домена
к сайту Ремиксера»*.

| Статус | Кому принадлежит | Что значит |
|---|---|---|
| **`provisioning`** | только КУПЛЕННОМУ у нас имени | имя регистрируется |
| **`connecting`** | обоим — и **единственный** такт своего домена | домен наводится на проект |
| **`propagating`** | только купленному | адрес настроен и расходится по интернету |

**Состояние `verifying` («Secure padlock is switching on») удалено.** Оно стояло последним
такт­ом на обоих путях, то есть свой домен, который человек просто подключает, зачем-то
проходил через выпуск сертификата. Стадия была нашей, а не продуктовой. Вместе с ней ушли
пресет консоли «Padlock switching on» и строка `verifying` из всех карт состояний.

Ось `registering` переименована в `provisioning`: код теперь говорит словарём дизайнера.
Копирайт трёх карточек — в `prototype/src/modules/publish/PublishPanel.tsx`.

---

## 🔴 Поправка 15.09.2026 — письмо о подтверждении почты идёт ПОСЛЕ подключения

Дизайнер, глядя на карточку подтверждения: *«мы его показываем только после того как домен
уже законекился, пока кастомный домен не законекчен, мы не показываем уведомление о почте»*.

**Что было.** `registrantUnconfirmed` = `isCustomDomainActive && icann`, то есть карточка
горела с первого такта прогона; а сам купленный прогон **парковался на `provisioning`**,
ожидая письма. Единственное, что требует действия от человека, жило ровно в том окне, где
продукт только что пообещал обратное: панель говорила «регистратор принял заказ, от вас
ничего не требуется» и сразу под этим — «адрес начнёт работать, когда подтвердите почту».
Два ответа на один вопрос, и действенный был тише.

**Что стало.** Гейт переехал в КОНЕЦ прогона.

| Такт | Почта не подтверждена |
|---|---|
| `provisioning` | карточки почты нет, письма нет |
| `connecting` | карточки почты нет, письма нет |
| `propagating` | карточки почты нет, письма нет |
| **`ready`** (парк) | **карточка почты и письмо — единственное на экране** |
| `live` | недостижимо, пока почта не подтверждена (`violations()`) |

Это не уступка в честности: три стадии — НАША половина работы (заказ, наведение,
расхождение), и она правда завершается. Неподтверждённая почта удерживает не подключение,
а **ответ адреса** — ровно то, что и значит `ready`: подключён, настроен, ещё не открыт.

**Следствия, которые пришлось закрыть вместе с правилом:**

1. Две двери в `live + icann` (пара, которую `violations()` объявляет невозможной): кнопка
   Publish в панели писала `domain: 'live'` из `ready` безусловно, и `settle` делала то же
   на ветке `finish` для `retryConnect` (attach-таймлайн без гейта). Публиковать при
   неподтверждённой почте МОЖНО — результат — сайт на бесплатном адресе, — поэтому двигаться
   не должен именно домен.
2. `published + ready` перестало быть нарушением, пока почта не подтверждена: с гейтом в
   конце это самая частая форма из всех (человек, который уже публиковал, покупает домен), и
   формулировка самого нарушения там не действует.
3. Селектор `isCustomDomainConnected` (`state/world.ts`) — «три стадии позади». На нём
   стоит `registrantUnconfirmed`, и панель читает селектор, а не свою копию предиката.

**Открыто:** на `ready + icann` топбар красит янтарный `working` «Setting up your domain», а
строка в окне доменов — «Connecting», хотя правило только что объявило подключение
законченным. Про адрес это правда, про соединение — нет. Пятого тона/лейбла на бордах нет.
