...
> 🔴 **Поправка 14.09.2026 к первому пункту «придержать».** Совет «не задерживаться на
> ICANN» был верен, **пока это был редкий риск на 16-й день**. По первопартийному ответу
> девелопера («Поправка 14.09.2026» в начале файла) это не риск, а **обязательный участок
> каждой первой покупки**: купленный домен не работает, пока клиент не кликнул в письме.
> На демо продукт-оунерам это не слайд про риски, а **шаг счастливого пути**, без которого
> демо покупки просто не доходит до работающего домена. Остальные пункты абзаца не тронуты.
...
- `docs/features/domains/states.md` — стейт-машина: имена, дословный EN-копирайт, глаголы.
  ...
  ICANN (§3.4).
  🔴 **И с 14.09.2026 всё наоборот в одном месте:** состояние `icann-verify` в `states.md`
  переписано ПРОТИВ этого документа — по первопартийному ответу девелопера DreamHost.
  При расхождении по этому состоянию правит `states.md`, а не §3.4.
