...
| **CMP-021** | The audit's **five** "why nobody can copy it" claims are refuted — the heading sits on all five overtake plays, §8.1–§8.5 (`grep -c '^\*\*Why nobody can copy it\.\*\*'` on the audit returns 5; the critique's "× 4" was an uncounted count, and the audit's §8 preamble already says so). Correct framing: **a 12–18 month head start on an undefended position.** Entri is a purchase order, Domain Connect is an open spec, Cloudflare Email Routing is free, and GoDaddy is conceded to be structurally capable. | refuted | 16 Aug 2026 | `synthesis-critique.md` §2 | POSITIONING throughout |
