...
**Correction to the "Staging subdomain" row above — the host is wrong (20 Aug 2026), and so is the one in Tier 2 item 19 and in §8.4.** The row prints `*.remixer.app`. The product's staging zone is **`remixer.ai`**: DreamHost's own KB describes the arrow beside **Publish to Staging** opening "your temporary website **ending in remixer.ai**", and the Remixer product page sells a free 30-day preview "**on a remixer.ai subdomain**" (`docs/product/FACTS.md` DH-302, verified 20 Aug 2026 — five agreeing search extractions, one of which volunteered the correction against a query naming only `.site` and `.app`). `remixer.app` was never sourced; neither was the `remixer.site` this project's own hand-off and prototype carried, which is why the two documents disagreed for weeks — **both sides were guessing**. What the correction does *not* settle is the **left-hand label**: whether staging reads `{project}.remixer.ai`, `{account}-{project}.remixer.ai` or a generated id is still unconfirmed, and one screenshot of the live builder closes it. The row's *substance* survives unchanged — an editable staging subdomain with a pencil affordance is still the proposal, and Base44 still buries the rename — only the zone was fiction. Logged in §10.1.
...
**Correction to item 19 above — the host in the signed preview URL is wrong (20 Aug 2026).** Read `*.remixer.ai`, not `*.remixer.app`: the zone is verified from DreamHost's own KB and product page (`docs/product/FACTS.md` DH-302; the same error runs through §6's "Staging subdomain" row and §8.4). The **label** in front of the zone remains unconfirmed, so treat any full address printed in this list as a placeholder shape. Nothing else in item 19 changes — the signed link, the 7-day expiry, the view-only mode and the "Preview — this is not the live site" banner are the proposal, and Base44's Testing Link is still the primitive being answered. Logged in §10.1.
...
**Correction to the sentence above — its second half is refuted (19 Aug 2026), and the correction travels with every play below.** The combination is not rare: **Wix — which owns Base44 — GoDaddy, IONOS and Hostinger all hold registrar + host + mail**, and Cloudflare hands out at-cost registration, the fastest one-click domain attach in the field, 300+ PoPs and **free inbound Email Routing** without being anyone's registrar of record (`synthesis-critique.md` §1 and §5). So read every **"Why nobody can copy it"** below — the heading is on all five plays, not the four the critique counted — as **"nobody has built it yet"**: Entri is a purchase order, Domain Connect is an open spec now on the IETF Standards Track (working group `dconn`, draft-03, 3 Jul 2026), and Cloudflare Email Routing is free. The connect teardown reaches the same verdict independently, from the live flows rather than from the org charts: **GoDaddy Airo already ships the same-house zero-record connect that §8.1 proposes** — *"If your domain is using GoDaddy nameservers, Airo AI Builder automatically updates your DNS"* — and Hostinger auto-connects Hostinger-registered domains too (`docs/features/domains/research/connect.md` §4.3, first-party docs). **The defensible version of the claim:** what we hold is placement and an installed base — **a 12–18 month head start on an undefended position**, not an impossibility. Nobody has put same-house connect *in the publish moment*, and no registrar in the AI cohort can be the registrar of record for a domain someone parked with us three years ago. That is a distribution advantage with a shelf life, and it argues for converting the installed base before it churns rather than for the word *uncopyable*. The original sentence is not quotable in a deck — logged in §10.1.

**Which plays this changes, and how they differ.** **§8.1** loses the word uncopyable and keeps the play: the mechanism is shipped elsewhere, the placement is not — and the critique adds a guard, because a DreamHost-registered domain sitting on external nameservers (usually Cloudflare) will not answer to records we write server-side, so "never mention nameservers" needs an explicit external-NS branch. **§8.2 is hit twice** and must stop being the headline: the mailbox is copyable (Cloudflare Email Routing is free, Google/Zoho/Microsoft resell mailboxes through APIs, and the real competitor is the Gmail the owner already uses), *and* the critique calls it the most dangerous item in this audit on its own merits — it inherits hosting's highest-volume, lowest-margin ticket class from users who by definition cannot debug it, puts novice mailboxes on shared sending reputation, needs abuse/KYC controls this audit never names, and Q17 already concedes it may cannibalise DreamHost mail revenue. Make it conditional on a deliverability-and-abuse plan, and fake-door the card before building it. **§8.3 is affected differently:** its narrow structural point survives — restoring a build is easy, restoring the *route* to it does require holding the zone — so what the critique attacks there is the feature, not the moat. A zone rollback silently breaks MX and third-party verification TXT records (Search Console, Stripe, social domain verification), which for a novice is a business-affecting, hard-to-diagnose footgun: "☑ Also restore DNS records" needs a far harder guard than a checkbox, and "within about 30 seconds" is a promise no one can keep across recursive resolvers. **§8.4** keeps its observation about Base44's crippled branches, but "a sampled row subset" breaks referential integrity and copying production data into a sandbox is a GDPR question. **§8.5** keeps its rows — expiry, certificate, DNS drift and mail alignment genuinely need the stack — and loses the frame: site health is a feature every host has shipped for a decade, and unprompted origin-error cards, at the resting error rate of any site on the public internet, would leave the panel permanently amber.
...
**Correction to the branch address above (20 Aug 2026).** `branch-name.yourproject.remixer.app` is built on a zone we do not own. The staging zone is **`remixer.ai`** (`docs/product/FACTS.md` DH-302, verified from DreamHost's KB and the Remixer product page), so the play's address reads `branch-name.yourproject.remixer.ai` — **and even that is half a guess**, because the *label* pattern is the unconfirmed half of DH-302: nobody has yet checked whether staging is `{project}.remixer.ai` at all, let alone whether a two-level `branch.project` form is available under it. Design the branch as a *distinct* address; do not treat this string as the spec. The mechanism the play rests on — forked schema, quarantined outbound mail, the three-group diff — is untouched by the correction. Logged in §10.1.
...
| Claim | Status |
|---|---|
| Registrar + host + mail "exists nowhere else in this field" (§8 preamble) — and, in the same words, "the last thing that is structurally uncopyable" (§1.3), "the only three-way structural position … in the category" (closing note) and the "Why nobody can copy it" heading on all five overtake plays | **Refuted** — by `synthesis-critique.md` §1 and §5, and independently by `docs/features/domains/research/connect.md` §4.3. **Wix (which owns Base44), GoDaddy, IONOS and Hostinger all hold all three**; Cloudflare gives away at-cost registration, the fastest one-click attach in the field and free inbound Email Routing. GoDaddy Airo already ships §8.1's same-house zero-record connect and Hostinger auto-connects its own domains, so "nobody can copy it" means "nobody has built it yet" — Entri is a purchase order, Domain Connect an open spec, Email Routing free. Quotable version: **a 12–18 month head start on an undefended position**, held by placement and a book of parked domains, not by structure |
| Our own staging host is `*.remixer.app` (§6 "Staging subdomain", Tier 2 item 19, §8.4's `branch-name.yourproject.remixer.app`) | **Refuted 20 Aug 2026 — the zone is `remixer.ai`.** DreamHost's KB ("your temporary website **ending in remixer.ai**") and the Remixer product page ("on a **remixer.ai** subdomain"), five agreeing extractions — `docs/product/FACTS.md` DH-302, restated in DH-005. `remixer.app` was never sourced, and neither was the `remixer.site` in this project's hand-off and prototype: two documents were comparing two guesses about our *own* product. **Still unverified: the left-hand label** (`{project}.remixer.ai`? a generated id?) — one screenshot of the live builder closes it, so no full staging address in this audit is quotable, only the zone |
