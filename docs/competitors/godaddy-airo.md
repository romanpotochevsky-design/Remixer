...
> Detail: [`audits/synthesis-q3-2026.md`](audits/synthesis-q3-2026.md) §4.3 "GoDaddy Airo — the
> strategic twin" ·
> [`../features/domains/research/connect.md`](../features/domains/research/connect.md) §4.3
> "Same-house zero-record — the rail nobody in the AI cohort has built"
>
...
assistant `[likely · no row]`.

Pricing — see FACTS **CMP-016** for the citable row. Tiers and prices were read off GoDaddy's
own pricing section `[verified · CMP-016 · Aug 2026]`; **one cell in this table is not verified**
and is marked in place:
...
playbook and the traps transfer directly (§4.3 "GoDaddy Airo — the strategic twin").
...
non-GoDaddy provider."* (`connect.md` §4.3 "Same-house zero-record")
`[verified · CMP-014 · 19 Aug 2026]`. **That is our play, already shipped.** Hostinger has it too,
for Hostinger-registered domains `[verified · CMP-015 · 19 Aug 2026]`.

**And they bury it.** The documented path is apps page → app → Domain → **Settings** → Connect
Domain → Yes, Continue — four clicks deep in a settings tree (`connect.md` §3 "Where the connect
entry point lives", §4.3 "Same-house zero-record") `[verified · CMP-014 · 19 Aug 2026]`.
...
select the app → Domain → Settings → Connect Domain → Yes, Continue (§4.3.2 "The ten legacy-host
traps", Trap 7) `[verified · CMP-014 · 19 Aug 2026]`.

So the honest version of our advantage is **placement, not plumbing**. The mechanism is copyable —
the critique was right about that (critique §2), and **CMP-021** carries the refutation. Two
separate sentences from `connect.md` say it, and they are in two different sections — quote them
apart, not welded together:

- §4.3 "Same-house zero-record — the rail nobody in the AI cohort has built": *"nobody has put it
  in the publish moment. The moat is the placement, not the plumbing."*
- §12 "What Remixer should do — six decisions", decision 6: *"it is a head start, not an
  impossibility."*

Nobody has put the zero-record case in the publish moment. That is what is ours, and it has a shelf
life, not a fence around it.
...
management, bulk checks, and expansion to OpenAI, Google and Perplexity
(§4.3.1 "The playbook worth copying") `[verified · no row]` — **this is the most strategically
loaded claim on the page and it has no register row; it needs one.**
...
- **The domain-first funnel, where the domain is the transaction and the site is the free gift.**
  ...
  one-page site delivered instantly (§4.3.1) `[verified · no row]`.
- **Named agents rather than one assistant.** Outcomes, not a tech stack — their CBO's framing:
  *"Small business owners do not want to master a tech stack; they want outcomes."* (§4.3.1)
  `[likely · no row]` — press framing, not a product observation.
- **The entry screen — the best single composition in the field.** Near-black violet ground, a
  ...
  `[likely · CMP-029 · Aug 2026]`. *Hex values are estimated from a marketing screenshot — the
  composition is usable, the numbers are not (**CMP-029**); see Do not repeat.*
- **Violet discipline.** Violet appears only on the BETA pill, alternating headline words, the prompt
  glow, the send buttons and the AI sparkle. Every non-AI control is neutral (§4.3.1)
  `[likely · no row]` — read off one marketing screen, so read it as a *rule* rather than a
  measurement. This is exactly the rule our indigo/purple should follow, and it independently
  matches the blue = action / violet = brand split measured in Lovable's live top bar (**DH-401**,
  **DH-402** for what our own blue actually is).
- **One published billing rule:** *"Credits are used when you perform an action that uses Airo AI
  ...
  (§4.3.1) `[verified · CMP-002 · Aug 2026]`.
...
- **Restore never auto-publishes.** Both History and Backups restore end with the identical
  ...
  restoring. This is the correct model and it is already our Tier 1 item #9 (§4.3.1)
  `[verified-doc · no row]`.
- **Expiry-first spend order, stated in the product:** credits are spent in order of expiration,
  free and paid alike (§4.3.1) `[verified-doc · no row]`. Critical for us because of our own
  first-month bonus (**DH-007**).
- **Downgrade as a soft landing** — the site goes offline but *"your content is preserved"* and is
  restorable on re-engagement (§4.3.1) `[verified-doc · no row]`.
- **Getty Images reachable from chat.** GoDaddy's own comparison tables score licensed stock against
  Base44 and Hostinger Horizons, both "AI-generated only" (§4.3.1) `[likely · no row]` — vendor
  marketing
...
1. **Brand sprawl.** GoDaddy maintains a help article titled *"Which GoDaddy website builder do I
   ...
   **Needing a disambiguation article is the diagnosis** `[verified-doc · no row]`.
2. **The credit balance exiled from the work surface** — User menu → Manage my plan → Total AI Credit
   ...
   GoDaddy keeps Airo's tucked away on a separate screen"* `[verified · CMP-038 · Aug 2026]`.
   **Our persistent
...
3. **IA that reveals its history** — a six-item mode bar containing **two adjacent asset managers**
   ("Menu" and "Media"), and a Settings screen holding exactly two things `[likely · no row]` —
   from documentation and one screenshot, not from inside the product.
4. **Label drift** — the same device selector is "Preview Desktop" in one doc and "Change device" in
   another; the same share affordance is "View & Share" and "Share link" `[verified-doc · no row]`.
   We will drift the same way as the rail grows unless there is a canonical string registry
   (§10.2 Q27; our answer is `../product/COPY-RULES.md` §7 "One name per thing").
5. **Dev-server internals leaking into a no-code product** — a manual "Refresh Preview" button, and
   ...
   server."* `[verified-doc · no row]`.
6. **Configuration by pasted prompt** — the official contact-form article tells users to paste
   GoDaddy-authored strings into chat `[verified-doc · no row]`. Non-discoverable, brittle, leaks
   internal agent architecture ("email skill"), and spends metered credits to change a setting.
7. **Leaving the builder to buy a domain** — see above. *"The single biggest unclaimed opening in the
   category."*
8. **Identity break at the door** — dark violet entry screen, **light** builder interior
   `[likely · no row]`. The product you fell for is not the product you use. Bolt does the same
   thing in reverse; we hold our shell background throughout (**DH-403**) and should keep doing so.
9. **Help pages wrapped in the storefront**, behind the full commercial mega-nav
   `[verified · no row]`.
10. **The renewal cliff** — first-year price advertised, materially higher renewal. The *pattern* is
    corroborated by several reviewers; **the specific renewal numbers are not**
    `[unverified · no row]` (see Do not repeat). Our own plan shape — no cliff — is **DH-001**, and
    it is a positioning weapon we are not using.

## Domain / publish behaviour

Zero-record automatic DNS for domains on GoDaddy nameservers, manual instructions at the user's
provider otherwise `[verified · CMP-014 · 19 Aug 2026]` — but reached only through four clicks of
settings, never at publish time (`connect.md` §3 "Where the connect entry point lives"). Buying a
domain ejects the user to the storefront. The Free tier **cannot publish at all**, and published
Airo projects land on `*.airoapp.ai` `[verified · CMP-022 · Aug 2026]` — that row is a `refuted`
one, and what it refutes is the *old* `godaddysites.com` claim; the `*.airoapp.ai` half is the
correction. Note also that the **Entri × GoDaddy agreement (June 2025) folds GoDaddy's Domain
Connect into Entri** — the two automation rails are converging, which matters for our own
build-vs-buy question (`connect.md` §4.1 "Entri — the bought rail"; §10.2 Q14)
`[verified · STD-012 · 19 Aug 2026]`. GoDaddy has mailboxes but **resells Microsoft 365** at
$7.99/mo, and its own comparison table concedes "Professional email: Yes — Purchase required"
...
| Topic | Section |
|---|---|
| The playbook worth copying — funnel, agents, entry screen, billing rule, restore model, Getty, MCP | `synthesis-q3-2026.md` §4.3.1 "The playbook worth copying" |
| The ten legacy-host traps, in full | §4.3.2 "The ten legacy-host traps" |
| Fact-check corrections that override the dossier | §4.3.3 "Fact-check corrections" |
| Verified pricing table and measured credit burn | §4.3.4 "Pricing and credit burn" |
...
| Our zero-record play, and why it needs rewriting | §8.1 "Zero-record go-live" + `connect.md` §4.3 "Same-house zero-record" |
| Same-house zero-record rail and connect entry depth | `connect.md` §4.3 "Same-house zero-record", §3 "Where the connect entry point lives", §12 decision 6 |
| **Entri × GoDaddy convergence** | `connect.md` **§4.1** "Entri — the bought rail" (not §4.2 — §4.2 is Domain Connect, and this pointer was wrong in two places until 20.08.2026); register row **STD-012** |
...
- **`godaddysites.com` free-tier subdomains — refuted** `[refuted · CMP-022 · Aug 2026]`. Airo's
  Free plan cannot publish at all, and
  ...
  wrong.
- **"SEO and code-quality scans are free" — refuted** `[refuted · CMP-023 · Aug 2026]`. Security
  and QA scans are free; **SEO and legal scans cost credits**; repairs always cost.
- **"No keyboard shortcuts or command palette", "no inline diffs, file chips or per-message revert" —
  unverifiable** `[unverified · CMP-029 · Aug 2026]`. GoDaddy's help and product pages returned
  HTTP 403 to fetching. Absence of documentation is not evidence of absence; this needs in-product
  verification.
- **Bimodal radii and the entry-screen hex values — unverifiable** `[unverified · CMP-029 · Aug
  2026]`. Sourced to a single marketing screenshot; pixel radii and exact hexes cannot be confirmed
  that way. Use the *composition* of that entry screen, not its numbers.
- **"Desktop-only as an authoring surface" — unverifiable** `[unverified · CMP-029 · Aug 2026]`.
  The quoted strings could not be confirmed; indirect corroboration only supports the weaker reading
  that a QA agent checks mobile responsiveness.
- **Airo Plus $59.88 → $95.88 and All Access → $323.88 renewals — third-party sourced,
  `[unverified · no row]`.** The renewal-cliff *pattern* is corroborated by multiple reviewers; the
  specific numbers are not. The Ultimate tier's 750-credit figure is likewise corroborated but not
  read off the tier card (**CMP-016**).
